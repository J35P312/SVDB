...
def main(args):
    ...
    with opener(args.query_vcf, "rt") as lines:
        for line in lines:
            ...
            current_variation = [chrA, int(posA), chrB, int(posB), event_type, FORMAT, line]
    ...
    if args.bedpedb or args.db:
        ...
        for line in f:
            ...
            DBvariants[chrA][chrB][event_type]["coordinates"].append(
                np.array([int(posA), int(posB)]))
            if "GT" in FORMAT and not Use_OCC_tag:
                DBvariants[chrA][chrB][event_type]["samples"].append(
                    np.array(FORMAT["GT"]))
        ...
        for query in queries:
            ...
            if not Use_OCC_tag:
                if query[5]:
                    content[7] = "{};{}={};{}={}".format(
                        content[7], args.out_occ, query[5], args.out_frq, (query[5] / float(db_size)))
            else:
                if query[5][0]:
                    content[7] = "{};{}={};{}={}".format(
                        content[7], args.out_occ, int(query[5][0]), args.out_frq, query[5][1])
        ...
        db_size = len(c.execute(A).fetchall())
        if not db_size:
...
def SQDB(Query_variant, args, c):
    ...
    A = 'SELECT {} FROM SVDB WHERE var == \'{}\' AND chrA == \'{}\' AND chrB == \'{}\' AND posA <= {} AND posA >= {} AND posB <= {} AND posB >= {}'.format(
        selection, variant["type"], variant["chrA"], variant["chrB"], variant["posA"] + distance, variant["posA"] - distance, variant["posB"] + distance, variant["posB"] - distance)
    hits = c.execute(A)
